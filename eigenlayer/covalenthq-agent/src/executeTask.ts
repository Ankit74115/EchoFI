...
async function createSignature(account: any, isSafe: boolean, contents: string) {
  ...
  const messageHash = keccak256(encodePacked(["bool", "string"], [isSafe, contents]));
  ...
  publicClient.watchEvent({
    ...
    onLogs: async (logs) => {
      for (const log of logs) {
        ...
        await respondToTask(walletClient, publicClient, contractAddress, account, task, taskIndex);
      }
    },
  });
